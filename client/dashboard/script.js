...
var topPicksList = new ReactiveVar([]);
...
Template.dashboard.onRendered(function() {
  ...
  setTimeout(function(){
    $("#bootstrap-touch-slider").carousel({interval: 10000});
  },1500);
  ...
  Meteor.call("getTopPicks", Session.get("storeId"), function(err,res){
    var aa = JSON.stringify(res);
    if(aa.indexOf(".webp") >= 0)
    {
      aa = aa.replace(".webp", ".png");
    }
    var aaa = JSON.stringify(Session.get("topPicksList"));      
    if(aa !== aaa) {
      Session.set("topPicksList",aa);
      setTimeout(function(){
        $('.itemstop').flyto({
          item      : 'li',
          target    : '.cartssa',
  ...
  Meteor.call("getSpritiousLiquors", Session.get("storeId"), function(err,res){
    var aa = JSON.stringify(res);
    if(aa.indexOf(".webp") >= 0)
    {
      aa = aa.replace(".webp", ".png");
    }
    var aaa = JSON.stringify(Session.get("spritiousLiquorList"));      
    if(aa !== aaa) {
      Session.set("spritiousLiquorList",aa);
      setTimeout(function(){
        $('.itemsliquor').flyto({
          item      : 'li',
          target    : '.cartssa',
...
Template.dashboard.helpers({
  advsliderProduct:function(){
    var prodSlider = advSliders1.find({}).fetch();
    //console.log("advsliderProduct", prodSlider);
    return prodSlider;
  },
  advsliderProductCount1:function(){
    var prodSlider = advSliders1.find({}).count();
    if(prodSlider == 1){
      return true;
    }
  },
  advsliderProductCount2:function(){
    var prodSlider = advSliders1.find({}).count();
    if(prodSlider == 2){
      return true;
    }
  },
  advsliderProductCount3:function(){
    var prodSlider = advSliders1.find({}).count();
    if(prodSlider > 2){
      return true;
    }
  ...
  {
    var data = advSliders1.find().fetch();
    var count = 0;
    var finalData = [];
    var temp = [];
    var w = $(window).width();
  ...
  advSliderImage:function(id){
    var img = advSliders1.findOne({_id:id});
    var url = img.advSliderImage;
    var imageUrl = url.split("upload");
    imageUrl = imageUrl[0] + "upload/c_pad,w_500,h_367" + imageUrl[1];
  ...
  isSelectedProduct: function(ProTypeId)
  {
    ...
    var selectCategory = Session.get("FilterArray");
    //console.log("selectCategory.productType",selectCategory.productType);
  ...
  isSelectedBrand: function(brandId)
  {
    ...
    categoryImage: function(category) {
      var category = categories.findOne({_id: category._id});
    ...
    bannerProductImage: function(){
      ...
      var banner = products.find({adBanner: true});
      if(banner.count() > 0)
      {
        banner = products.findOne({adBanner: true});
    ...
    // getPostersList: function(){
    //   return posters.find().fetch();
    // },
    getPosters: function(){
      var data = posters.find({},{limit:3}).fetch();
      return data;
    },
    posterFirst: function(){
      ...
      {
        if(typeof posters.find({},{limit:3}).fetch()[0] !== "undefined")
        {
          if(typeof posters.find({},{limit:3}).fetch()[0].posterImage !== "undefined")
          {
            return posters.find({},{limit:3}).fetch()[0].posterImage;
          }
        }
      }
    },
    posterFirstId: function(){
      var data = posters.find({},{limit:3}).fetch();
      if(typeof data !== "undefined")
      {
        if(typeof posters.find({},{limit:3}).fetch()[0] !== "undefined")
        {
          if(typeof posters.find({},{limit:3}).fetch()[0]._id !== "undefined")
          {
            return posters.find({},{limit:3}).fetch()[0]._id;
          }
        }
      }
    },
    ...
    posterSecond: function(){
      ...
      if(typeof data !== "undefined")
      {
        if(typeof posters.find({},{limit:3}).fetch()[1] !== "undefined")
        {
          if(typeof posters.find({},{limit:3}).fetch()[1].posterImage !== "undefined")
          {
            return posters.find({},{limit:3}).fetch()[1].posterImage;
          }
        }
      }
    },
    posterSecondId: function(){
      var data = posters.find({},{limit:3}).fetch();
      if(typeof data !== "undefined")
      {
        if(typeof posters.find({},{limit:3}).fetch()[1] !== "undefined")
        {
          if(typeof posters.find({},{limit:3}).fetch()[1]._id !== "undefined")
          {
            return posters.find({},{limit:3}).fetch()[1]._id;
          }
        }
      }
    },
    ...
    posterThird: function(){
      ...
      if(typeof data !== "undefined")
      {
        if(typeof posters.find({},{limit:3}).fetch()[2] !== "undefined")
        {
          if(typeof posters.find({},{limit:3}).fetch()[2].posterImage !== "undefined")
          {
            return posters.find({},{limit:3}).fetch()[2].posterImage;
          }
        }
      }
    },
    posterThirdId: function(){
      var data = posters.find({},{limit:3}).fetch();
      if(typeof data !== "undefined")
      {
        if(typeof posters.find({},{limit:3}).fetch()[2] !== "undefined")
        {
          if(typeof posters.find({},{limit:3}).fetch()[2]._id !== "undefined")
          {
            return posters.find({},{limit:3}).fetch()[2]._id;
          }
        }
      }
    },
    ...
    imageResize: function(url) {
      var imageUrl = url.split("upload");
      imageUrl = imageUrl[0] + "upload/c_pad,h_617,w_1242" + imageUrl[1];
      imageUrl = imageUrl.replace("http://","https://");
      var ext = imageUrl.substr(imageUrl.lastIndexOf(".")+1);
      if(ext == "webp")
      {
    ...
    spirituousProduct: function(){
      var a = JSON.parse(Session.get("spritiousLiquorList"));
      return a;
    },
    topPicks: function(){
      var a = JSON.parse(Session.get("topPicksList"));
      return a;
    },
    collectionForYou: function(){
      var orderList = orders.find({userId: Meteor.userId(), status: "pending"},{sort:{createdAt:-1}}).fetch();
      if(orderList.length>0)
      {
        ...
        for(var i = 0; i < orderList.length; i++)
        {
          ...
          for(var j = 0; j < orderProducts.length; j++)
          {
            ...
            if(productsQueue.indexOf(singleProduct.id) == -1)
            {
              ...
              // console.log("singleProduct.id",singleProduct.quantity);
        ...
        for(var j = 0; j < productsQueue.length; j++)
        {
          var categoryId = products.findOne({_id: productsQueue[j]});
          // console.log("categoryId",categoryId);
          if(categoryId != null)
          {
            categoryId = categoryId.categoryType;
            // console.log("productsQueue[j]",productsQueue[j]);
            var sameCatProds = products.findOne({_id:{$ne:productsQueue[j]}, categoryType: categoryId});
            if(sameCatProds !== undefined){
              collectionListing.push(sameCatProds);
              if(j == 3)
                break;
            }
        ...
        // console.log("collectionListing",collectionListing);
        setTimeout(function(){
        $('.itemscollection').flyto({
          item      : 'li',
          target    : '.cartssa',
          button    : '.my-btn'
        });
    ...
    if(typeof prodQuantity !== "undefined")
    {
...
Template.dashboard.events({
  ...
  'change #category': function (e) {
    ...
    // //console.log("categoryType val",val);
    if(val == "")
    {
 ...
 'click .advItem': function(e)
  {
    e.preventDefault();    
    var advItemId = $(e.target).data('advimg'); 
    //console.log("advItemId",advItemId);
    if(advItemId != null)
    {
      var advSlider = advSliders1.findOne({_id: advItemId});
      //console.log("poster",advSlider);
      if(advSlider.productRadio)
      {
        //console.log("productRadio",advSlider.productRadio);
        productsArray = advSlider.productsType;
        if(productsArray != undefined){
          Session.set('category', undefined);
          Session.set("productsArray", productsArray);
          Session.set("PreviousPage", "/dashboard");
          Router.go("/advSliderProducts/" + advItemId);
        }        
      }
      else if(advSlider.categoryRadio)
      {
        //console.log("categoryRadio",advSlider.categoryRadio)
        category = advSlider.categoryType;
  ...
  'click .bannerItem': function(e)
  {
    e.preventDefault();
    var id = $(e.target).data("id");
    var bannerDetails = banner.findOne({
      "_id": id
    });
    //console.log("bannerDetails",bannerDetails.productIds);
    if(bannerDetails.productIds != null || bannerDetails.productIds != undefined)
    {
      Session.set("PreviousPage", "/dashboard");
      Router.go("/bannerproducts/" + id);
      return;
    }
    else{
      return;
  ...
  //       var s = res.text;
  //       s = s.replace(/^0+/, '');
  //       var data = products.findOne({"productId": s});
  //       if(data != null)
  //       {
  ...
  'click #addCart':function(e){
    ...
    Meteor.subscribe("getSpecificProducts", [$(e.target).data("attr")], function(){
      var a = products.findOne({_id: $(e.target).data("attr")});
   var cartRecord = carts.findOne({userId: Meteor.userId()});
   if(cartRecord != null)
   {
     var cartProdStore = cartRecord.storeId;
   }
   else if(cartRecord == null || cartRecord == undefined ){
    var cartProdStore = Session.get("storeId");  
   }
   var selectedProdStore = a.storeName;
   if(Session.get("storeId") == undefined)
   { 
     if(a != null)
     {
   ...
   else if(cartProdStore == selectedProdStore)
   {
     var storeId = cartProdStore;
   }
   ...
   {
     if(presentProduct !== null && presentProduct !== undefined)
     {
     ...
     {
       Meteor.call("createCart", productId, userId, a.storeName, function(err, res){
         if(err){
          //console.log("test123",err.reason);
           alert(err.reason);
         }
         else{
           if(res == "false"){
  ...
  'click #favProduct': function (e) {
    ...
    Meteor.subscribe("getSpecificProducts", [productId], function(){
      if(Session.get("storeId") == undefined)
      {
        var storeId = products.find({_id: productId});
        if(storeId.count() > 0)
        {
          storeId = storeId.fetch()[0];
  ...
  'click #categoryImageClick': function(e)
  {
    e.preventDefault();
  ...
  'click #posterImageToProduct': function(e)
  {
    e.preventDefault();
    var posterId = $(e.target).data('poster');    
    if(posterId != null)
    {
      poster = posters.findOne({_id: posterId});
      //console.log("poster",poster);
      if(poster.productRadio)
      {
        //console.log("productRadio",poster.productRadio);
        productsArray = poster.productsType;
        if(productsArray != undefined){
          Session.set('category', undefined);
          Session.set("productsArray", productsArray);
          Session.set("PreviousPage", "/dashboard");
          Router.go("/posterProducts/" + posterId);
        }        
      }
      else if(poster.categoryRadio)
      {
        //console.log("categoryRadio",poster.categoryRadio)
        category = poster.categoryType;
        if(category != undefined){
          // console.log("category",category);
          Session.set('category', undefined);
          Session.set('category', category);
