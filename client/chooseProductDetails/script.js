Template.chooseProductDetails.onCreated(function() { 
    // Meteor.subscribe('getProducts');
    Meteor.subscribe('getReviews');
});
Template.chooseProductDetails.onRendered(function() {
  $("body").css("overflow","auto");
});
Template.chooseProductDetails.helpers({
  ...
  productName: function(){
    ...
    Meteor.subscribe('getSpecificProducts',productID);
    if(products.findOne({ _id: Router.current().params._id}) != null)
    {
      var product = products.findOne({ _id: Router.current().params._id});
...
Template.chooseProductDetails.events({
